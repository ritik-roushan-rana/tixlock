...
export function OccupancyByEventChart({ events }: { events: DashboardEventRow[] }) {
  ...
  // An empty state rather than `return null`. Returning null dropped the card out of
  // the grid, so a new organiser saw a dashboard with a hole in it and no explanation —
  ...
  if (data.length === 0) {
    return (
      <Card>
        ...
        <CardContent>
          <p className="py-12 text-center text-sm text-muted-foreground">
            No showings scheduled yet. Occupancy appears here once an event has a show.
          </p>
        </CardContent>
      </Card>
    );
...
export function ShowRevenueChart({ shows }: { shows: EventReportShow[] }) {
  ...
  // Same reasoning as the occupancy chart: an explained empty card beats a missing one.
  if (data.length === 0) {
    return (
      <Card>
        <CardHeader className="pb-2">
        ...
        <CardContent>
          <p className="py-12 text-center text-sm text-muted-foreground">
            No showings yet. Add a show to this event to see per-showing revenue.
          </p>
        </CardContent>
      </Card>
    );
